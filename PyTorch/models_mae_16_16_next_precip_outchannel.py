...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, precip_model, img_size=(720,1440), patch_size=(8,8), in_chans=20,
        ...
        # manxin todo
        self.head = nn.Linear(decoder_embed_dim, self.patch_size[0] * self.patch_size[1], bias=False)
        # --------------------------------------------------------------------------
...
# todo manxin rewrite pos_emb initial method
    ...
    def forward_decoder(self, x, ids_restore, precip_flag):
        # embed tokens
    ...
    def forward(self, imgs, mask_ratio=0.75, precip_flag=False):
        latent, mask, ids_restore = self.forward_encoder(imgs[0], mask_ratio)
        pred = self.forward_decoder(latent, ids_restore, precip_flag)  # [N, L, p*p*3]
        precip_pred = self.precip_model(pred)
        # print(precip_pred.shape)
        # print("precip_pred.shape")

        loss = self.forward_loss(imgs[1], precip_pred, mask,mask_ratio)
        # loss = self.forward_loss(imgs[1], pred, mask,mask_ratio)
        # return loss, pred, mask
        return loss, precip_pred
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        embed_dim=768, depth=15, num_heads=12,
        decoder_embed_dim=512, decoder_depth=16, decoder_num_heads=8,
