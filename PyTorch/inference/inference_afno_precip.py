...
from afnonet_8 import AFNONet, PrecipNet
import wandb
...
def load_model_mae(args, ckpt_path, flag_wind=False):
    ...
    if flag_wind == True:
        model = models_mae_afno_8_8_finetune_next_precip.__dict__[args.model](norm_pix_loss=norm_pix_loss, img_size=args.img_size,
                                                                patch_size=args.patch_size)
    else:
        model = models_mae_afno_8_8_finetune_next_precip_outchannel.__dict__[args.model](precip_model=precip_model, norm_pix_loss=args.norm_pix_loss, img_size= args.img_size, patch_size = args.patch_size)
...
def setup(params):
    ...
    checkpoint_file_precip = params['best_checkpoint_path_precip']
        ...
        for i in range(valid_data.shape[0]):
            if i == 0:  # start of sequence
                ...
                if orography:
                    ...
                    save_image(future_pred_tp[0][0], "/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/infer_/" + "TS" + "_" + str(i)+ ".png")
                    save_image(future_tp[0][0], "/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/infer_/" + "GT_TS" + "_" + str(i)+ ".png")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--yaml_config", default='./config/AFNO_afno_8_8_finetune_precip.yaml', type=str)
    parser.add_argument("--config", default='precip', type=str)  # full_field
    parser.add_argument("--override_dir",
                        default='/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/ckpt/',
                        type=str,
                        help='Path to store inference outputs; must also set --weights arg')
    ...
    # Set up directory
    for iii in range(3, 28):
        ff = open(
            r"/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/inference_for_valid.txt",
            "a", encoding="UTF-8")
        ff.write(str(iii) + '\n')
        flag_mx = 'true'
        ...
        args.weights = '/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/ckpt/checkpoint-' + str(
            iii) + '.pth'
        ...
        params['best_checkpoint_path_precip'] = args.weights if args.override_dir is not None else os.path.join(expDir,
                                                                                                         'training_checkpoints/best_ckpt.tar')
        params['best_checkpoint_path'] = args.pretrained_ckpt_path if args.override_dir is not None else os.path.join(expDir,
                                                                                                         'training_checkpoints/best_ckpt.tar')
