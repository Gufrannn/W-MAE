    ...
    for data_iter_step, (samples) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                ...
                # pred_two, _ = model(samples, mask_ratio=args.mask_ratio)
                # save_image(rearrang_v1(args, pred_two)[0][0],
                #            args.save_dir + "/" + str(epoch) + "_" + str(data_iter_step) + "_twostep.png")

            #
            # samples[1] = samples[1].to(device, non_blocking=True)
            # # precip_pred = precip_model(pred)
            # # pred, _ = model(samples, mask_ratio=args.mask_ratio, precip_flag=True)

            # pred = rearrang_v0(args, pred)
            # print(pred_two.shape)
            ...
            # precip_out = precip_model(samples)
            # loss = loss_obj(precip_out, samples[1])

            # print(precip_out.requires_grad)
            # print("precip_out.requires_grad")


            # pred_two = rearrang_v2(args, pred_two)
            # precip_out = precip_model(pred_two.detach())
            # save_image(precip_out[0][0],
            #            args.save_dir + "/" + str(epoch) + "_" + str(data_iter_step) + "PredPrecip.png")

            # precip_out = rearrang_v1(args, precip_out)
            # precip_out = patchify(args, precip_out)
            # precip_out = rearrang_v1(args, precip_out)



            # fcn_loss = LpLoss()
            # loss = fcn_loss(precip_out, samples[1])
            # loss = forward_loss(args, samples[1], precip_out)
            # print(pred.shape)
            # print(pred.shape)
