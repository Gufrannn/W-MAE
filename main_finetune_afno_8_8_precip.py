...
import models_mae_afno_8_8_finetune_next_precip
import models_mae_afno_8_8_finetune_next_precip_outchannel
from engine_finetune_afno_8_8_next_precip import train_one_epoch\
# , valid_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir',
                        default='/home/manxin/codes/fcn/mae/mae-main/output_dir',
...
# manxin
    ...
    parser.add_argument('--run_mode', default='', type=str)
...
def main(args):
    ...
    model = models_mae_afno_8_8_finetune_next_precip.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, img_size= args.img_size, patch_size = args.patch_size)
    model_precip = models_mae_afno_8_8_finetune_next_precip_outchannel.__dict__[args.model](precip_model=precip_model, norm_pix_loss=args.norm_pix_loss, img_size= args.img_size, patch_size = args.patch_size)

    model.to(device)
    model_precip.to(device)

    model_without_ddp = model
    model_precip_without_ddp = model_precip
    ...
    param_groups = optim_factory.add_weight_decay(model_precip_without_ddp, args.weight_decay)

    optimizer = torch.optim.AdamW(param_groups, lr=0.00025, betas=(0.9, 0.95))
    optimizer_precip = torch.optim.AdamW(param_groups, lr=args.lr_new, betas=(0.9, 0.95))

    loss_scaler = NativeScaler()
    loss_scaler_precip = NativeScaler()
    ...
    # if args.resuming:
    #     precip_ckpt_path = args.checkpoint_path
    # else:
    #     ckpt_path = args.pretrained_ckpt_path
    #     use_pretrained_ckpt = True
    args.resuming = True
    wind_ckpt_path = args.pretrained_ckpt_path
    precip_ckpt_path = args.checkpoint_path
    args.resuming = False

    misc.load_model_v1(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler,
                       ckpt_path=wind_ckpt_path)
    args.resuming = False
    # args.resuming = True
    misc.load_model_v1(args=args, model_without_ddp=model_precip_without_ddp, optimizer=optimizer_precip, loss_scaler=loss_scaler_precip,
                        ckpt_path=precip_ckpt_path)
    # args.resuming = True
    # if use_pretrained_ckpt == True:
    #     args.start_epoch = 0
    #     args.resuming = False
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model_without_ddp, model_precip_without_ddp, train_data_loader_h5,
            optimizer_precip, device, epoch, loss_scaler_precip,
            log_writer=log_writer,
            args=args
        )
        if args.save_dir:
            misc.save_model(
                args=args, model=model_precip, model_without_ddp=model_precip_without_ddp, optimizer=optimizer,
...
if __name__ == '__main__':
    ...
    args.run_num = 'afno_8_precip' # 'p_1'
    ...
    args.pretrained_ckpt_path = '/home/lichangyu/data/codes/mae/mae-main/output_dir/afno_8_precip/finetuning/ckpt/checkpoint-1000.pth'
    # p_qkv_16_16_next_finetune_577/pretraining/ckpt  best  cur
    args.output_dir = '/home/lichangyu/data/codes/mae/mae-main/output_dir'
    args.yaml_config = './config/AFNO_afno_8_8_finetune_precip.yaml'
    # args.output_dir = '/home/manxin/codes/FourCastNet/NVLabs-FourCastNet/FourCastNet-master/mae/mae-main/output_dir'
    ...
    params['checkpoint_path'] =  os.path.join(save_dir, 'ckpt/checkpoint-1000.pth')  # manxin todo aaa
